...
- **After-journey additions (round 3).** A jump-link nav (`#after-nav`, also linked from the depth nav). `#bill`: live estimates from the prompt (list price at $3/$15 per million, node-time share at $32/node-hour with 50k tok/s prefill, 70 tok/s decode, batch 48; 12 kW node → Wh; 1.1 mL/Wh water) plus a latency waterfall (wire 40, gate 10, prefill 50+0.2/token, first 20, decode tokens/70, render 15 ms). `#scale`: six rungs request → GPU → node → rack → building → grid, linking to Data Center Builder rather than duplicating it. `#makers`: sample companies per layer. Existing sections got ids `#open-vs-closed`, `#numbers`, `#privacy`. All figures labelled as 2026 estimates.
- **Polish round.** Open Graph and Twitter meta, canonical URL, inline SVG favicon, 1200×630 social card at `docs/social-card.png` (rendered from scene 6 by a Playwright script). Sources list, keyboard hint, credits and MIT license in the footer. Phones: diagram text defaults to 1.25× unless chosen, scenes switch as a card passes the pinned stage's bottom edge, a step pill shows position where the depth nav is hidden. Keyboard: arrows or J/K step, 1–8 jump. The step 8 follow-up question is editable and re-tokenizes.
- **Readability at large text.** Bubbles and captions wrap to their available width at the current text multiplier. At A++ (1.5×) the stage hides its detail group and secondary labels (`plain-hide`, `xl-hide`) and shows the plain-language labels instead, so what remains stays legible; the cards keep all content. A+ (1.25×) keeps details but swaps a few long strings for compact ones. Phones default to A+. The after-journey nav is sticky and highlights the section in view.
